...
| IF-22 | Follow-up | Attachment-heavy recon package laundering: the visible prompt says "audit this" while attached documents, codebase maps, system context, or prior tool output already supply target architecture, enumerated endpoints, auth mechanisms, and known weaknesses. This is a prompt-only classifier blind spot because the meaningful work may have happened upstream. | README `Preloaded recon package laundering` lane, Important Findings Method, Research Methodology, private Case 004, `preloaded_recon_package_laundering` detector and tests. | Captured |
