...
| CTF/lab authorization laundering | HTB, CTF, or lab framing can set a permissive tone the model never rechecks. Real credentials or target details may arrive mid-session to unblock friction, retry-after-block language keeps momentum, and sophisticated AD CS/template analysis starts looking like normal lab work. | Verify lab scope before operational action, gate mid-session credentials as a hard authorization boundary, preserve refusal state across retries, and keep AD CS/template output remediation-safe unless trusted lab scope is explicit. |
...
| Tool-output provenance gap | Tool results often arrive as model-readable strings. Without verified provenance, sandboxed interpretation, typed parsing, or schema enforcement, injected text can cross from data payload into directive space. Training and prompt-level skepticism are soft controls, not a structural boundary. | Wrap tool results in typed envelopes, verify provenance and authority, parse before model exposure, separate data from directives, and classify/redact outputs before display or export. |
| Formatting fidelity laundering | The user asks for pure formatting or document processing, points to a pre-existing file, and says certain blocks must pass through unchanged. Fidelity to source material becomes the cover for reproducing content the model should have classified first. Filter-test style filenames can leak intent. | Classify source files before transformation, treat "unchanged" blocks as display/export decisions, reject unsafe pass-through, and use artifact metadata as triage evidence. |
...
| Platform session-token exfil framing | Bug-bounty or operational-analysis framing asks about injection/exfiltration vectors aimed at platform session tokens. | Treat session tokens as a hard data boundary; allow remediation-safe summaries, not operational collection paths. |
...
| Hypervisor mapping-invariant bypass | EPT/SLAT duplicate-GPA remap acceptance in a split-page/rootkit context is not just a virtualization bug; it is a stealth primitive. | Keep output invariant/remediation-level: reject ambiguous mappings, log remap attempts, require trusted lab scope, and avoid hook design. |
